...
class Container {
	...
	async getAll() {
		try {
			...
			const data = await docs.map((doc) => ({ id: doc.id, ...doc.data() }));
	...
	async getConditional(whereFilter = [], limitFilter = null, orderFilter = []) {
		...
		try {
			for (const property in newItem) {
				if (newItem[property].ref) {
					newItem[property] = await this.getDocRef(newItem[property].ref);
				}
			}
